...
class _TrackOrder extends State<TrackOrder> {
  ...
  void _deleteSo(String docId, String distrId, MainModel model, String storeId,
    ...
    final http.Response responseI = await http.post(Uri.parse(
        'http://mywayegypt-api.azurewebsites.net/api/updatedelap/$docId/$distrId/$storeId/$soType'));

    if (responseI.statusCode == 200) {
      final http.Response responseII = await http.post(Uri.parse(
                                  ...
                                  title: ListTile(
                                    ...
                                    trailing: Container(
                                      child: Column(
                                        children: <Widget>[
                                          Text(
                                            'جنيه ${(formatter.format(double.tryParse(firstSorder[index].soTotal)))}',
                                            style: TextStyle(
                                              fontWeight: FontWeight.bold,
