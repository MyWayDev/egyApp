...
class Tickets extends StatefulWidget {
  ...
  final bool isAdmin;
  final List stores;
  Tickets({@required this.distrId, this.isAdmin, this.stores});
...
class _TicketsState extends State<Tickets> {
  ...
  List<Ticket> storeTickets = [];
  List<TicketType> types = [];
  List<Store> stores = [];
  ChatScreen msgs;
  final String path =
      "flamelink/environments/egyProduction/content/support/en-US";
  ...
  void userLoad() {
    storeTickets = ticketsData
        .where((o) => widget.distrId == int.tryParse(o.member))
        .toList();
    filteredTickets = storeTickets.reversed
        .where((o) =>
            o.open == true ||
  ...
  void adminLoad() {
    storeTickets = ticketsData
        .where((t) => widget.stores.where((e) => t.store == e).any((a) => true))
        .toList();
    filteredTickets = storeTickets.reversed
        .where((o) =>
            o.open == true ||
  ...
  Widget build(BuildContext context) {
    !widget.isAdmin ? userLoad() : adminLoad();
    // filteredTickets
    ...
    return Scaffold(
        ...
        floatingActionButtonLocation:
            !widget.isAdmin ? FloatingActionButtonLocation.endDocked : null,
        body: Column(
          children: <Widget>[
            Padding(
                                          ...
                                          title: Row(
                                            children: [
                                              ...
                                              Padding(
                                                  padding: EdgeInsets.only(
                                                      left: 25)),
                                              Text(filteredTickets[index].store)
                                            ],
                                          ),
                                          subtitle: Row(
                                            children: <Widget>[
...
//!add .where before foreach to filter open and closed tickets in different pages
    ...
    searchResult = filteredTickets
        .where((ticket) =>
                ticket.open.toString().contains(text) ||
                ticket.store.contains(text) ||
          ...
          ? Container(
              child: TextButton(
                style: ElevatedButton.styleFrom(
                    ...
                    primary: !ticket.open ? Colors.green[50] : Colors.pink[50],
  ...
  _getTicketTypes() async {
    DataSnapshot snapshot = await database
        .reference()
        .child('flamelink/environments/egyProduction/content/ticketType/en-US/')
        .once();
  ...
  _getStores() async {
    DataSnapshot snapshot = await database
        .reference()
        .child('flamelink/environments/egyProduction/content/stores/en-US/')
        .once();
  ...
  _asyncInputDialog(BuildContext context) async {
    return showDialog<String>(
      ...
      builder: (BuildContext context) {
        return TicketSelect(
            stores, types, widget.distrId.toString().padLeft(8, '0'));
      },
    );
  }
