...
import '../pages/order/end_order.dart';

class MainModel extends Model {
  // ** items //** */
  static Firebase firebase;
  static const String _version = '5.2R1'; //!Modify for every release version./.
...
//Morc update items code

  Future<List<Item>> fbItemListIndo() async {
  ...
  Future<List<Item>> dbItemsListIndo() async {
    List<Item> products;
    //List productlist;
    final response = await http.get(
        Uri.parse('http://mywayindoapi.azurewebsites.net/api/allitemdetails'));
  ...
  Future<List<Item>> fbItemsUpdateFromDbIndo() async {
    ...
    final List<Item> dbItems = await dbItemsListIndo();
    final List<Item> fbItems = await fbItemListIndo();
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      try {
        if (items[i].promo != '0' || items[i].promo != null) {
          var promoString = storageRef
              .child('tag-${items[i].promo}-tag.png')
              .getDownloadURL();
    ...
    void updateItemsToFirebaseIndo(int id, Item item) {
    ...
    for (Item item in items) {
      updateItemsToFirebaseIndo(item.id, item);
      print(
  ...
  Future<List<Item>> fbItemListMorc() async {
    DataSnapshot snapshot = await FirebaseDatabase.instance
        .reference()
        .child('flamelink/environments/production/content/items/en-US/')
        .once();
    Map<dynamic, dynamic> fbitemsList = snapshot.value;
    List fblist = fbitemsList.values.toList();
    List<Item> fbItems = fblist.map((f) => Item.fromList(f)).toList();
  ...
  Future<List<Item>> dbItemsListMorc() async {
    List<Item> products;
    //List productlist;
    final response = await http
        .get(Uri.parse('http://mywayapi.azurewebsites.net/api/allitemdetails'));
    if (response.statusCode == 200) {
      final productlist = json.decode(response.body) as List;
  ...
  Future<List<Item>> fbItemsUpdateFromDbMorc() async {
    if (Firebase.apps.isEmpty) {
      await Firebase.initializeApp(
        name: 'mobile-coco',
        options: const FirebaseOptions(
    ...
    final List<Item> dbItems = await dbItemsListMorc();
    final List<Item> fbItems = await fbItemListMorc();
    /*final FirebaseStorage storage = FirebaseStorage(
        app: app, storageBucket: 'gs://mobile-coco.appspot.com/');*/
    ...
    void updateItemsToFirebaseMorc(int id, Item item) {
      DatabaseReference ref = FirebaseDatabase.instance.reference().child(
          'flamelink/environments/production/content/items/en-US/${id.toString()}');
      ref.update(item.toJsonUpdate());
    }

    for (Item item in items) {
      updateItemsToFirebaseMorc(item.id, item);
      print(
          '${item.id}..${item.itemId}..${item.price}..${item.bp}..${item.promo}..${item.promoImageUrl}');
    }
    return items;
...
// End Morc Update Item Fb code.......
  ...
  Future<List<BackOrder>> getBulkBackOrderItems(
    ...
    final response = await http
        .get(Uri.parse('$testPath/getBackOrderItemsNew/$distrId/$storeId'));
  ...
  Future<List<Products>> getitemDetailsApi() async {
    ...
    //List productlist;
    print('RUNNING....getitemDetailsApi ');
    final response = await http.get(Uri.parse('$testPath/allitemdetails'));
    if (response.statusCode == 200) {
      final productlist = json.decode(response.body) as List;
  ...
  Future<DistrBonus> distrBonus(String distrId) async {
    ...
    final response =
        await http.get(Uri.parse('$testPath/deserve_bonus/$distrId'));
    if (response.statusCode == 200) {
  ...
  void updateGuestPriceFB() async {
    List<Products> products = await getitemDetailsApi();
    if (products.isNotEmpty) {
      products.forEach((f) {
        updateGPrice(f.itemId);
      });
    }
  }

  void updateGPrice(String itemId) async {
    final DatabaseReference dbRef = FirebaseDatabase.instance.reference();
    final DatabaseReference reference = dbRef.child(
        'flamelink/environments/egyProduction/content/items/en-US/$itemId');
    DataSnapshot snapshot = await reference.once();
    var price = snapshot.value['price'];
    var guestPrice = price * 1.35;
    await reference.update({'guestPrice': guestPrice});
    print('$itemId..=>$guestPrice');
  }
...
//!--------*Settings*-----------//
  ...
  Future<List<Item>> fbItemsUpdateFromDb() async {
    ...
    for (Item item in items) {
      ...
      print(
          '${item.id}=>${item.held}..${item.itemId}..${item.price}..${item.bp}..${item.promo}..${item.promoImageUrl}');
    }
    return items;
  }
...
//!--------*
  ...
  void addItemOrder(Item item, int qty, [bool _held = false]) {
    final ItemOrder itemorder = ItemOrder(
      ...
      guestPrice: double.parse(item.guestPrice.toString()),
      bp: item.bp,
  ...
  bool iheld(int x, {String item}) {
    if (itemorderlist.isNotEmpty) {
      if (item != null) {
        ...
        var s = searchResult.where((s) => s.itemId == item);
        int index = itemData.indexOf(i.first);
        int sIndex = searchResult.indexOf(s.first);
...
//!--------*
  ...
  int getItemIndex(int x) {
    var item = searchResult.length == 0
        ? itemData.where((i) => i.itemId == itemorderlist[x].itemId)
        : searchResult.where((i) => i.itemId == itemorderlist[x].itemId);

    int index;
    item.isNotEmpty
        ? index = searchResult.length == 0
            ? itemData.indexOf(item.first)
            : searchResult.indexOf(item.first)
        : null;
...
//!--------*
  ...
  double orderWeight() {
    ...
    if (itemorderlist.isNotEmpty) {
      ...
      x += giftorderList.length > 0 ? promoWeight() + giftWeight() : 0.0;
      notifyListeners();
    }
    return x;
  }
...
//!--------*
  double orderSum() {
    double x = 0;
    for (ItemOrder i in itemorderlist) {
      x += !user.isGuest ? i.price * i.qty : i.guestPrice * i.qty;
    }
    notifyListeners();
    return x;
  }
...
//!--------*legacy salesOrder*---------//
  ...
  Future<List<Sorder>> checkSoDeletion(String userId) async {
    ...
    final http.Response response =
        await http.get(Uri.parse('$testPath/userpending/$userId'));
    if (response.statusCode == 200) {
  ...
  Future<DateTime> serverTimeNow() async {
    DateTime _stn;
    final http.Response response =
        await http.get(Uri.parse('$testPath/datetimenow'));
    if (response.statusCode == 200) {
...
//!--------*Gift*---------//
  ...
  Future<List<Promo>> promoList() async {
    ...
    for (var x in promos) {
      var serverT = intl.DateFormat('yyyy-MM-dd').format(serverTime);

      var promoF =
          intl.DateFormat('yyyy-MM-dd').format(DateTime.parse(x.fromDate));
      var promoT =
          intl.DateFormat('yyyy-MM-dd').format(DateTime.parse(x.toDate));
...
//!--------*Stock*---------//
  Future<int> getStock(String itemId) async {
    ...
    http.Response response =
        await http.get(Uri.parse('$testPath/stock/$itemId/$setStoreId'));
    print(response.body);
    if (response.statusCode == 200) {
  ...
  void showLicenseAgreement(BuildContext context, MainModel model) {
    showDialog(
      context: context,
        ...
        return AlertDialog(
          content: SingleChildScrollView(
            child: Column(
              children: <Widget>[
                Container(
                  height: 60.0, // desired height
                  width: 155.0, // desired width
                  child: Image.asset(
                    'assets/images/mwlogo.png',
                    fit: BoxFit.cover,
                  ),
                ), // Assuming your logo is named 'logo.png' and is in the 'assets' directory
                SizedBox(height: 20),
                Text(
                  html2md.convert(settings.guestMsg ?? ''),
                  style: TextStyle(
          ...
          actions: <Widget>[
            ...
            TextButton(
              ...
              onPressed: () {
                Navigator.of(context).pop();
                Navigator.push(context, MaterialPageRoute(builder: (_) {
                  return NewMemberPage(model, isGuest: model.user.isGuest);
                }));
                // Add your agreement logic here
                //
              },
            ),
          ],
  ...
  Future<String> getOrderInvalidPerc(MainModel model) async {
    ...
    if (orderBp() > 100) {
      if (promoSum > settings.promoSumPerc && _exPromo > 0 && _wPromo > 0) {
        _msg = '% ' + '${promoSum.toInt().toString()}' + ' ' + 'العرض و الشاور';
      } else if (_exPromo > settings.exitemsPerc && _wPromo == 0) {
        promoSum = _exPromo;
        _msg = '% ' + '${_exPromo.toInt().toString()}' + ' ' + 'الشاور';
      } else if (_wPromo > settings.weeklyPromoPerc && _exPromo == 0) {
        promoSum = _wPromo;
...
//? Areaupdate to firebase..
  ...
  Future<List<Area>> getArea() async {
    final response = await http.get(Uri.parse('$testPath/areas'));
...
//return couriers;
  ...
  Future<List<AreaPlace>> getAreaPlace() async {
    ...
    final response =
        await http.get(Uri.parse('$testPath/get_all_shipment_places/'));
    if (response.statusCode == 200) {
...
//!--------*
  ...
  List<Store> stores;
  Future<List<Store>> getStores({bool isEnabled = true}) async {
    DataSnapshot snapshot =
    ...
    stores = isEnabled
        ? list
            ...
            .toList()
        : list.map((f) => Store.json(f)).toList();
    return stores;
  }
...
//!--------*Users/Members*-----------//
  ...
  Future<User> nodeJson(String nodeid) async {
    http.Response response =
        await http.get(Uri.parse('$testPath/memberid/$nodeid'));
...
//!------------*
  ...
  Future<void> guestActivated(String key) async {
    DatabaseReference databaseReference = FirebaseDatabase().reference();
    databaseReference.child('$pathDB/guest/en-US/$key').update({
      "isAllowed": true,
    }).whenComplete(() async => await guestDetails(key));
  }
...
//!--------*
    ...
    if (password == '0penk' //'0penk0ngpls'
      ...
      if (_userInfo != null) {
        if (_userInfo.isAllowed) {
          ...
          //getArea();
          try {
            firebaseUser = await firebase_auth.FirebaseAuth.instance
  ...
  Future<bool> guestLogIn(BuildContext context) async {
    User _userInfo =
        await userData('6').catchError((e) => print('Erro:${e.toString()}'));
    ...
    if (_userInfo != null) {
      if (_userInfo.isAllowed) {
        ...
        try {
          firebaseUser = await firebase_auth.FirebaseAuth.instance
              .signInWithEmailAndPassword(
                  email: _userInfo.email, password: 'abouauf');
        } catch (e) {
          print('singin error caught:${e.toString()}');
          return false;
        }
  ...
  void versionControl(BuildContext context) {
    databaseReference =
        database.reference().child('$path/lockScreen/en-US/build');
    databaseReference.onValue.listen((event) async {
      String bulid = await event.snapshot.value;
      //print('APPLOCKED-XXXXX:$appLocked');
      if (bulid != _version) {
        itemorderlist.clear();
        giftorderList.clear();
        promoOrderList.clear();
        print('$bulid CheckYour Version $_version :)');
        signOut();
        Navigator.pushNamedAndRemoveUntil(context, '/login', (_) => false);
  ...
  Future<bool> distrVerification(String distrId) async {
    ...
    if (response.statusCode == 200 && response.contentLength > 0) {
      ...
      if (_verDistr.distrId.substring(1, 2) == '3' && _verDistr != null) {
        http.Response responseBool = await http.get(Uri.parse(
  ...
  Future<bool> leaderVerification(String distrId) async {
    ...
    http.Response response = await http.get(
        Uri.parse('$testPath/leaderverification/${userInfo.distrId}/$distrId'));
  ...
  String updatePassword(email, String pw) {
    String _msg = ' تمت العمليه';

    try {
      print('${firebaseUser.user.email}');
      firebaseUser.user.updatePassword(pw); //(firebaseUser.email);
    } catch (e) {
      _msg = 'خاطأ فى العمليه';
...
//!--------*
  ...
  Future<bool> emailSignIn(String email, String password) async {
    ...
    try {
      ...
      print(' Future signIn display ${firebaseUser.user.uid}');
    } catch (e) {
      print('singin error caught:${e.toString()}');
      return false;
    }
