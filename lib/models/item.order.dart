...
class ItemOrder {
  ...
  Map<String, dynamic> toJsonGuest() => {
        "ITEM_ID": itemId,
        "QTY": orderSatus ? qty * -1 : qty,
        "QTY_REQ": qty,
        "UNIT_PRICE": price,
        "NET_PRICE": guestTotalPrice,
  ...
  String postToJson(ItemOrder itemOrder, [bool isGuest = false]) {
    final dyn = isGuest ? itemOrder.toJsonGuest() : itemOrder.toJson();
...
class SalesOrder {
  ...
  Future<http.Response> createPost(SalesOrder salesOrder) async {
    final response = await http.put(
        Uri.parse('http://mywayegypt-api.azurewebsites.net/api/invoice'),
        headers: {
          HttpHeaders.contentTypeHeader: 'application/json',
          //HttpHeaders.authorizationHeader: ''
        },
